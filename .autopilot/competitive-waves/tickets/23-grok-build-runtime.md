...
**Волна:** после прогона
**Status:** **declined 2026-09-08 решением пользователя** — «грок нам пока не нужен, мы будем
пользоваться лишь claude, codex». Разведка ниже сохранена целиком: она проверена против
апстрима и переживёт решение, если рантаймовый набор когда-нибудь расширится.

Прямое следствие этого же решения, уже поставленное: профиль `cross-verifier@1.0` на codex
(коммит ниже). Именно он снимает претензию к правилу W5 — до него предупреждение было
безответным, потому что все встроенные профили разрешались в `claude-code`.
