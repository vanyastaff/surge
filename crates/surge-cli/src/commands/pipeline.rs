    ...
    if resume {
        if let Ok(store_path) = surge_persistence::store::Store::default_path() {
            if store_path.exists() {
                if let Ok(store) = surge_persistence::store::Store::open(&store_path) {
                    // Get the most recent checkpoint for this spec
                    if let Ok(checkpoints) = store.list_task_states_by_spec(spec_file.spec.id) {
                        if let Some((_, state, _)) = checkpoints.first() {
                            if let surge_core::state::TaskState::Executing { completed, total } = state {
                                if *completed > 0 {
                                    ...
                                    if let Ok(graph) = surge_spec::DependencyGraph::from_spec(&spec_file.spec) {
                                        if let Ok(batch_ids) = graph.topological_batches() {
