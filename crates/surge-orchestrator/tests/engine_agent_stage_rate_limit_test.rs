...
/// `StageError::RateLimited.runtime` normalizes through the same registry
...
/// must report a *normalized* `runtime`, not the profile's raw
...
/// from.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent_stage_with_profile_registry_reports_normalized_runtime() {
...
/// special-cased for `AgentKind` derivation (`derive_agent_kind_from_id`'s
/// `agent_id == "mock"` arm) but is not itself a registry id or alias —
/// `Registry::normalize_agent_id("mock")` is `None`. The bundled
/// `bundled/profiles/mock-1.0.toml` carries exactly this shape, so this is
/// a real, shipped case, not a synthetic one.
fn drop_profile_with_mock_runtime(profiles_dir: &std::path::Path) {
    std::fs::create_dir_all(profiles_dir).unwrap();
    std::fs::write(
        profiles_dir.join("mock-role-1.0.toml"),
        r#"
schema_version = 1
...
category = "agents"
description = "Test profile targeting the mock runtime"
when_to_use = "Tests"
...
/// Review finding #4: a profile that resolves but whose `agent_id` the
/// registry does not recognize must not silently lose its identity on
/// either write site — `StageError::RateLimited.runtime` *and*
/// `SessionOpened.agent_id` must both fall back to the **raw** id rather
/// than reporting `None` (which reads as "no profile registry wired at
/// all", a different and misleading fact). Without this, every run on the
/// bundled `mock` profile would report `CapacityStatus::Unclassified`
/// where a raw-preserving reader would report `Known`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent_stage_with_unregistered_mock_runtime_falls_back_to_raw_id_not_none() {
    let profiles_dir = tempfile::tempdir().unwrap();
    drop_profile_with_mock_runtime(profiles_dir.path());
    let disk = DiskProfileSet::scan(profiles_dir.path()).unwrap();
    let registry = Arc::new(ProfileRegistry::new(disk));
