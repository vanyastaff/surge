...
impl HubTab {
    fn all() -> &'static [Self] {
        &[Self::Installed, Self::Available, Self::Benchmarks]
    }
}
            ...
            .child(
                div()
                    ...
                    .child(if is_active {
                        ...
                        format!(
                            "idle · last {}",
                            agent.last_seen.as_deref().unwrap_or("recently")
                        )
                    } else {
        ...
        div()
            ...
            .when(agent.capabilities.models.is_some(), |el: Div| {
                let rows: Vec<Div> = agent
                    ...
                    .collect();
                el.child(section_label("Models"))
                    .child(section_card(div().v_flex().children(rows)))
            })
            ...
            .when(agent.capabilities.permissions.is_some(), |el: Div| {
                ...
                el.child(section_label("Permissions"))
                    .child(section_card(content))
            })
            // ── Stats ── (label outside)
            ...
            .child(
                div()
                    ...
                    .child(stat_card_with_period(
                        ...
                        "avg",
                        latency_color(agent.avg_latency_ms),
                    )),
            )
                ...
                match &agent.usage {
                    Usage::ClaudeCode {
                        ...
                        section = section
                            ...
                            .child(usage_bar(
                                "Weekly Quota",
                                &format!("{:.0}% used · resets {weekly_reset}", weekly_pct * 100.0),
                                *weekly_pct,
                                wk_color,
                            ))
                            .child(
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .child(if *is_local {
                                                ...
                                                "Inherited from provider API".to_string()
                                            }),
                                    ),
                            )
                            .child(
...
impl Render for AgentHubScreen {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .child(div().px_4().pt_3().pb_2().child(self.render_tabs(cx)))
            // Content
            .child(match self.active_tab {
                HubTab::Installed => {
                    let configured = self.configured();
                    let items: Vec<Stateful<Div>> = configured
                        .iter()
                        .enumerate()
                        .map(|(i, a)| self.render_agent_item(i, a, cx))
                        .collect();
